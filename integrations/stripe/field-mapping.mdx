...
## Summary: RootFi Unified API vs Stripe RawData

This section provides a clear summary of which Sales Tax fields come from RootFi's normalized fields versus Stripe's raw data.

### Fields from Stripe RawData

| Entity | Sales Tax Field | RawData Path | Why RawData? |
|--------|-----------------|--------------|--------------|
| Customer | Ship To Address Line 1 | `raw_data.shipping.address.line1` or `raw_data.line1` | Stripe address structure preserved |
| Customer | Ship To City | `raw_data.shipping.address.city` or `raw_data.city` | Stripe address structure preserved |
| Customer | Ship To State | `raw_data.shipping.address.state` or `raw_data.state` | Stripe address structure preserved |
| Customer | Ship To Postal Code | `raw_data.shipping.address.postal_code` or `raw_data.postal_code` | Stripe address structure preserved |
| Customer | Ship To Country | `raw_data.shipping.address.country` or `raw_data.country` | Stripe address structure preserved |
| Transaction | Ship To Address Line 1 | `raw_data.shipping.address.line1` or `raw_data.line1` | Stripe address structure preserved |
| Transaction | Ship To City | `raw_data.shipping.address.city` or `raw_data.city` | Stripe address structure preserved |
| Transaction | Ship To State | `raw_data.shipping.address.state` or `raw_data.state` | Stripe address structure preserved |
| Transaction | Ship To Postal Code | `raw_data.shipping.address.postal_code` or `raw_data.postal_code` | Stripe address structure preserved |
| Transaction | Ship To Country | `raw_data.shipping.address.country` or `raw_data.country` | Stripe address structure preserved |

### Fields from RootFi Unified API

| Entity | Sales Tax Field | RootFi Field |
|--------|-----------------|--------------|
| Customer | Name | `name` |
...
| Transaction | Transaction Date | `posted_date` |
...
| Entity | Sales Tax Field | Value | Notes |
|--------|-----------------|-------|-------|
| Product | SKU | `""` (empty) | Stripe products don't have SKU in RootFi mapping |
| Product | Tax Code | `""` (empty) | Must be configured manually |
| Line Item | Product SKU | `""` (empty) | Not available from Stripe |
| Line Item | Product Taxability Code | `""` (empty) | Except shipping items: `SRV_Shipping` |
| Transaction | Ship From Address | `""` (empty) | Not mapped from Stripe |

### Address parsing code reference

The Stripe address parsing logic in `sync.address_parser.ts`:
...
### Product mapping code reference

The Stripe product mapping in `stripe.transformer.ts`:
