...
| marketplace | false | [Marketplace](#marketplace) | Marketplace name if the transaction is from a marketplace. |
| tax_remitter_type | false | [Tax Remitter Type](#tax-remitter-type) | Indicates who is responsible for collecting and remitting tax on this transaction. |
| customer_business_identification_number | false | string | Business identification number for the customer on this invoice (e.g., VAT number, EIN, or GST number). Must be consistent across all line items within the same invoice. |
...
Indicates the marketplace platform through which the transaction was facilitated, if applicable.

Currently the following options are supported: `AMAZON`.

#### Tax Remitter Type

Indicates who is responsible for collecting and remitting tax on this transaction.

Currently the following options are supported: `MARKETPLACE_FACILITATOR`, `SELLER`, `BUYER`.