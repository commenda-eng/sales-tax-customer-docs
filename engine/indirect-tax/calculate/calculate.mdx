...
- **Field presence** — fields marked as required for a country (e.g., `address_line_1`, `city`, `state`, and `postal_code` for US) must be provided and non-empty.
- **State code validation** — when provided, the state must match one of the valid ISO 3166-2 subdivision codes for the country.
- **Postal code format** — when provided, the postal code must match the country's expected format (e.g., `^\d{5}(-\d{4})?$` for US ZIP codes).
